...
- function declaration vs function expression
  ...
  - `IIFE` is an inline function expression.
- _note_ : `arrow function` if you used `{}` then you should use the word `return` to return something
- when you use a function that do a calculation process, you should put the invocation of this function in a variable and not invocing it many time => **for performance**, ex:
...
- in `arrow functions` =>
  - `this` is **elasticaly lexical scoped**, as they define `this` based on `where` it was written
...
| addEventListener can do multible things when event is triggered                                        | onclick can only do one task when event is triggered                                        |
...
// (5) selecting all elements that lives in our virtual-DOM and convering it to an array so that we can compare it with the real-DOM
const newElements = Array.from(newDOM.querySelectorAll("*"));

// (6) selecting all elements that lives in our real-DOM and convering it to an array so that we can compare it with the virtual-DOM
