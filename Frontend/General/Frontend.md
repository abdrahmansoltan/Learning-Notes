...
![Browser Overview](./img/browser-overview.png)
...
1. **URL Access**: User types a URL (e.g., `https://www.google.com/maps`).
   - **DNS Lookup**: It does a mapping from the domain name to the IP address of the server via DNS (Domain Name System).
     - This IP address is used to locate the server on the internet which contains the requested resource.
     - (e.g., `https://216.58.211.206:443`).
     - Note that this step is **cached** by the browser and DNS servers to improve performance.
   - **Port**: Identifies specific service on the server.

2. **TCP/IP Handshake**: Establish a connection between browser and server. (Happens on a **3-way handshake/round-trip**)
   ![TCP/IP Handshake](./img/tcp-ip-handshake.png)
   - **TCP**: Breaks data into packets for transmission and reassembles them at the destination.
     > 💡 There's something called **TCP Slow Start** which is a congestion control algorithm used by TCP to avoid network congestion (when too many packets are sent at once, the network can become overwhelmed and start dropping packets) by gradually increasing the amount of data sent over the network until it finds the optimal transmission rate.
     > ![TCP Slow Start](./img/tcp_slow_start.png)
     >
     > The file we're focusing on here is the `html` file so TCP Slow Start helps ensure that the HTML file is delivered efficiently without overwhelming the network.
   - **IP**: Routes packets to ensure they reach the destination.

3. **TLS Negotiation**: Establish a secure connection between browser and server by ensuring data privacy and integrity between the client and server.

4. **HTTP Request**: Sent by the browser to the server.
...
5. **HTTP Response**: Server sends data back to the browser.
   - It includes the status code (e.g., `200 OK`, `404 Not Found`), headers and body.

6. **Request/Response Cycle**: Multiple requests for assets (e.g., `js`, `css`, `img`) are made.
   - Browser makes new HTTP requests for each asset/file
   - Limited simultaneous requests to avoid slowing down the connection.
   - Files are rendered once all assets are received. **(The browser uses the `content-type` header to determine how to process each file**)
...
![BrowserEngine](./img/BrowserEngine.png)
