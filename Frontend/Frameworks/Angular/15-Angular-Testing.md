...
When testing NgRx, you typically focus on testing the state management logic, including actions, reducers, selectors, and effects. You can use the `@ngrx/store/testing` module to create a mock store for unit testing components that interact with the store.

- Most of the complex logic will be in the **reducers** which should be heavily tested
  - Usually the tests check the state before and after an action is dispatched to ensure the reducer updates the state correctly.
